import history_icon from './history_icon.png'
import menu_icon from './menu_icon.png'
import plus_icon from './plus_icon.png'
import question_icon from './question_icon.png'
import setting_icon from './setting_icon.png'
import bulb_icon from './bulb_icon.png'
import compass_icon from './compass_icon.png'
import gallery_icon from './gallery_icon.png'
import mic_icon from './mic_icon.png'
import user_icon from './user_icon.png'
import youtube_icon from './youtube_icon.png'
import message_icon from './message_icon.png'
import code_icon from './code_icon.png'
import send_icon from './send_icon.png'
import gemini_icon from './gemini_icon.png'

import { TfiPencil } from "react-icons/tfi";

export const assets = {
    history_icon,
    menu_icon,
    plus_icon,
    question_icon,
    setting_icon,
    compass_icon,
    bulb_icon,
    gallery_icon,
    mic_icon,
    user_icon,
    youtube_icon,
    message_icon,
    code_icon,
    send_icon,
    gemini_icon
}

export const data = [
    {
        id: 1,
        suggestion: `Brainstorm ideas for a mocktail given specific ingredients`,
        icon: <TfiPencil />,
        reactIcon: true,
    },
    {
        id: 2,
        suggestion: `Give me a beginner's guide to an activity`,
        icon: <TfiPencil />,
        reactIcon: true,
    },
    {
        id: 3,
        suggestion: `Create an image of an intergalactic event`,
        icon: <TfiPencil />,
        reactIcon: true,
    },
    {
        id: 4,
        suggestion: `Evaluate and rank common camera categories`,
        icon: bulb_icon,
        reactIcon: false,
    }
]
